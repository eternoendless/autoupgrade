...
class UpgraderCore
{
        ...
        if ($feed) {
            ...
            foreach ($feed->channel as $channel) {
                ...
                foreach ($channel as $branch) {
                    ...
                    if (in_array($this->channel, $followed_channels)
                        ...
                        if (in_array($this->channel, $array_no_major)
                            && version_compare(
                                $branch_name,
                                $this->branch,
                                '>'
    ...
    public function getApiAddons($xml_localfile, $postData, $refresh = false)
    {
        ...
        if ($refresh
            || !file_exists($xml_localfile)
            || @filemtime($xml_localfile) < (time() - (3600 * Upgrader::DEFAULT_CHECK_VERSION_DELAY_HOURS))
        ) {
    ...
    public function getXmlFile($xml_localfile, $xml_remotefile, $refresh = false)
    {
        ...
        if ($refresh
            ...
            $xml_string = Tools14::file_get_contents(
                $xml_remotefile,
                false,
    ...
    public function getXmlMd5File($version, $refresh = false)
    {
        return $this->getXmlFIle(
            _PS_ROOT_DIR_.'/config/xml/'.$version.'.xml',
            $this->rss_md5file_link_dir.$version.'.xml',
            $refresh
        );
    }
